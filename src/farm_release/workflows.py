...
from . import configuration as contract
...
def check(case, draft):
    ...
    for side in ['trigger', 'action']:
        ...
        for value in draft[side+'_fields']:
            ...
            elif kind == 'trigger_output' and (side != 'action' or not isinstance(source.get('ingredient_slug'), str) or source.get('ingredient_slug') not in ingredient_slugs):
