"""
LangGraph StateGraph for FARM Multi-Agent Resolution

Defines the WoT (Web of Things) multi-agent resolution workflow
using LangGraph StateGraph with Granite 4 native features.

Agentic Flow:
    START -> intent_analyzer -> load_candidates -> cross_scorer -> trigger_agent
          -> action_agent -> [ACCEPT: verifier -> END | REJECT: fallback -> END | FAILED: END]

Components:
    - Intent Analyzer: Decomposes query, extracts search intents
    - Cross-Scorer: Ranks all trigger-action pairs by compatibility
    - Trigger Agent: Selects trigger and makes OFFER
    - Action Agent: Validates and generates bindings
    - Verifier: Evaluates applet with reflection
    - Fallback: Handles retries with priority queue
"""
...
from agents.state import ResolutionState, create_initial_state
...
def create_resolution_graph(use_mock: bool = False, use_simple: bool = False, use_intent_analyzer: bool = True) -> StateGraph:
    """
    Create the resolution StateGraph.

    Args:
        use_mock: If True, use mock candidates instead of RAG
        use_simple: If True, use simple (non-LLM) agent implementations
        use_intent_analyzer: If True, include Intent Analyzer (default: True for full agentic)
    ...
    intent_analyzer_fn = intent_analyzer_simple if use_simple else intent_analyzer_node
    ...
    if use_intent_analyzer:
        graph.add_node("intent_analyzer", intent_analyzer_fn)
    ...
    if use_intent_analyzer:
        ...
        graph.add_edge("intent_analyzer", "load_candidates")
    else:
    ...
    def fallback_router(state):
        if state.get("resolution_status") == "failed":
            return "end"
        return "retry"
...
def create_agentic_graph(use_mock: bool = False) -> StateGraph:
    """
    Create the FULL AGENTIC resolution graph.
    ...
    return create_resolution_graph(
        ...
        use_intent_analyzer=True,  # Include Intent Analyzer
    )
...
def run_resolution(
    ...
    """
    Run the complete resolution workflow.
    ...
    Args:
        ...
        use_intent_analyzer: If True, include Intent Analyzer (default: True)
    ...
    if verbose:
        print(f"{'='*60}")
        print(f"FARM WoT Multi-Agent Resolution")
        print(f"{'='*60}")
        print(f"Query: {query}")
        print(f"Mode: {'Agentic' if use_intent_analyzer else 'Simple'}")
        print(f"{'='*60}\n")
    ...
    if verbose:
        ...
        for event in compiled.stream(initial_state, config):
            for node_name, node_output in event.items():
                ...
                if node_output.get("intent_analyzer_reasoning"):
                    reasoning = node_output['intent_analyzer_reasoning']
                    print(f"\n  REASONING:")
                ...
                if node_output.get("resolution_status"):
                    status = node_output['resolution_status']
                    print(f"\n  Status: {status.upper()}")
...
def display_final_applet(state: Dict[str, Any]) -> None:
    """
    Display the final applet in a formatted output.

    Args:
        state: Final resolution state containing the applet
    """
    ...
    rounds = metadata.get("resolution_rounds", 0)
    critique = metadata.get("verifier_critique", "")

    print(f"  Score:    {score:.2f}")
    print(f"  Status:   {'Executable' if is_exec else 'Needs Review'}")
    print(f"  Rounds:   {rounds} resolution attempt(s)")
    if critique:
...
def run_agentic_resolution(
    query: str,
    verbose: bool = True,
    show_applet: bool = True,
) -> Dict[str, Any]:
    """
    Run the FULL AGENTIC resolution with all Granite 4 features.

    This is the recommended entry point for production use.

    Features:
    - Intent Analyzer with Chain-of-Thought
    ...
    Example:
        result = run_agentic_resolution("When darkness detected, log to spreadsheet")
        print(f"Success: {result.get('resolution_status') == 'accepted'}")
...
def run_resolution_streaming(
    query: str,
    thread_id: Optional[str] = None,
    use_mock: bool = False,
):
    """
    Generator that yields state updates during resolution.
    ...
    """
    graph = create_resolution_graph(use_mock=use_mock)
...
def create_selector_graph(use_mock: bool = False, use_simple: bool = False, use_reference: bool = False) -> StateGraph:
    """
    Create the selector-based graph with multi-candidate scoring.

    Flow:
        intent_analyzer -> load_candidates -> trigger_selector -> action_selector -> verifier
        On low verifier score: selector_fallback -> try next best pair
    ...
    # Add nodes
    graph.add_node("intent_analyzer", intent_analyzer_node if not use_simple else intent_analyzer_simple)
    graph.add_node("load_candidates", load_fn)
...
def llm_fallback_node(state: ResolutionState) -> Dict[str, Any]:
    """
...
def selector_fallback_node(state: ResolutionState) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "resolution_status": "negotiating",  # Reset status
    }


def run_selector_resolution(
    query: str,
    verbose: bool = True,
    show_applet: bool = True,
) -> Dict[str, Any]:
    """
    Run the NEW DESIGN resolution with multi-candidate scoring.
...
def run_selector_with_reference(
    ...
    """
    Run selector resolution using reference test data (no RAG needed).
