...
EXPECT = {
    ...
    "M3_null_values":     ["tcov", "acov", "both"],
    "M4_false_values":    ["tcov", "acov", "both"],
    "M5_garbage_values":  ["grounded_rate", "stat_ok_rate"],
    "M6_scramble_fields": ["tcov", "acov", "both"],
    "M7_flip_source":     ["grounded_rate", "stat_ok_rate"],
...
print(f"{'mutation':<24}{'tcov':>8}{'acov':>8}{'both':>8}{'grnd':>8}{'stat':>8}   verdict")
print("-"*88)
def fmt(v): return "  n/a  " if v is None else f"{v:>7.3f}"
print(f"{'M0_baseline':<24}{fmt(base['tcov'])}{fmt(base['acov'])}{fmt(base['both'])}"
      f"{fmt(base['grounded_rate'])}{fmt(base['stat_ok_rate'])}   (reference)")
...
for name in MUTATIONS:
    ...
    print(f"{name:<24}{fmt(r['tcov'])}{fmt(r['acov'])}{fmt(r['both'])}"
          f"{fmt(r['grounded_rate'])}{fmt(r['stat_ok_rate'])}   {verdict}")
