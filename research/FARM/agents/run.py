...
Entry point for running the negotiation system.
...
from agents.metrics import get_metrics_tracker, NegotiationMetrics
...
def run_single_query(
    ...
    """
    Run negotiation for a single query.
...
def print_applet(applet: dict):
    ...
    print(f"  Negotiation Rounds:  {metadata.get('negotiation_rounds', 'N/A')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="FARM Agentic AI - Contract-based negotiation for TAP synthesis",
  ...
  python -m agents.run --eval queries.json --output results.json
    ...
    if args.json:
        ...
        if applet:
            ...
            print("=" * 60)
            print(f"Status: {result.get('negotiation_status', 'Unknown')}")
            if result.get("error"):
