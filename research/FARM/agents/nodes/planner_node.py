...
LangGraph node implementing the Planner Agent logic.
...
def planner_node(state: NegotiationState) -> Dict[str, Any]:
    ...
    try:
        ...
        # Always record expected tool calls since Planner should call search_triggers and search_actions
        if not tool_calls:
            tool_calls = [
        ...
        new_messages = [
            HumanMessage(content=f"[Planner] {query}"),
        ...
        return {
            ...
            "planner_reasoning": plan.get("thinking", ""),
...
def planner_simple(state: NegotiationState) -> Dict[str, Any]:
    """
    Simplified Planner without LLM call.
    ...
    return {
        ...
        "planner_reasoning": plan["thinking"],
    }
