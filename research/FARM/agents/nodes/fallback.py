"""
Fallback Logic for Agent Negotiation
====================================
...
# Local PAIR_ORDER for backward compatibility with old negotiation graph
...
def fallback_node(state: NegotiationState) -> Dict[str, Any]:
    ...
    return {
        ...
        # Reset current negotiation state
        "current_offer": None,
...
def should_continue(state: NegotiationState) -> Literal["continue", "verify", "end"]:
    """
    Routing function for the negotiation graph.

    Determines the next step based on negotiation status:
    - "continue": Go to fallback and try next pair
    - "verify": Proceed to Contract Verifier
    - "end": Negotiation complete (success or failure)
...
def get_current_pair_info(state: NegotiationState) -> Dict[str, Any]:
    """
...
def format_rejection_summary(state: NegotiationState) -> str:
    """
