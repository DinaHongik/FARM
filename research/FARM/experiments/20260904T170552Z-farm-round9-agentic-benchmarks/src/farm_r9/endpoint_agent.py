    ...
    *,
    ...
    for ordinal in range(1, maximum_calls + 1):
        result = client.chat(
            ...
            messages=messages, temperature=0.0, seed=42,
            # Granite's local Ollama manifest explicitly rejects the thinking
            # option; Cloud reasoning models accept the bounded `low` setting.
            think="low" if client.cloud else None,
        )
        call_record = {
