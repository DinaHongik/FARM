...
from experiment import hierarchical_log_probs
...
def load(path):
    model=SentenceTransformer(str(path),device='cuda:0',
                 ...
                 processor_kwargs={'fix_mistral_regex':False},config_kwargs=saved_config_overrides(path))
...
def smoke_reload(run, derived, config, phases):
    ...
    for phase in phases:
        ...
        for role in roles:
            ...
            actual=model.encode_query(RELOAD_SENTINELS,normalize_embeddings=True,convert_to_tensor=True).cpu()
...
def paired_comparisons(run, groups, hits, config):
    project=run.parents[3]
    old=project/'experiments'/'20260831T171222Z-farm-four-pipelines'/'results'
    paths={'e0_service':old/'e0_dev.json'} if config['arm']=='s1' else {'e3_function':old/'e3_dev.json'}
    previous=project/'experiments'/'20260905-five-training-techniques'/'arms'
    paths['previous_s1' if config['arm']=='s1' else 'previous_f3']=previous/('s1' if config['arm']=='s1' else 'f3')/'results_dev.json'
    if config.get('negative_refresh'):
        paths['matched_fixed_control']=run.parent/f"f3_fixed_seed{config['seed']}"/'results_dev.json'
    result={}
...
def evaluate(run, derived, config, validation_only=False):
    ...
    hard_predictions,unguided_predictions={},{}
    if service:
        ...
        heads=nn.ModuleDict({s:ServiceHead(prototypes[s],config['temperature']) for s in SIDES})
        ...
        for side in SIDES:
            ...
            vectors[side]=(q,d);scores[side]=(1./config['temperature'])*q@d.T
            if config['arm']=='h6':
                state_root=checkpoint_path(run,phase) if validation_only else run/phase/'final'
                state=(torch.load(state_root/'state.pt',map_location='cpu',weights_only=False)['heads']
                       if validation_only else torch.load(state_root/'heads.pt',map_location='cpu',weights_only=True))
                head=ServiceHead(prototypes[side],config['temperature']).eval()
                head.load_state_dict({k.removeprefix(side+'.'):v for k,v in state.items()})
                doc_services=np.array([r['service'] for r in corpus[side]])
                with torch.no_grad():
                    svc=head(torch.from_numpy(q))
                    raw=scores[side]
                    chosen=svc.argmax(-1).numpy()
                    ids=np.array([r['id'] for r in corpus[side]])
                    hard_predictions[side]=np.array([np.lexsort((ids,-np.where(doc_services==chosen[i],row,-np.inf)))[0] for i,row in enumerate(raw)])
                    unguided_predictions[side]=np.array([np.lexsort((ids,-row))[0] for row in raw])
                    scores[side]=hierarchical_log_probs(torch.from_numpy(raw),svc,doc_services).numpy()
            del model;gc.collect();torch.cuda.empty_cache()
    for side in SIDES:
    ...
    for index,group in enumerate(groups):
        ...
        if config['arm']=='h6':
            hard=tuple(int(hard_predictions[s][index]) for s in SIDES)
            unguided=tuple(int(unguided_predictions[s][index]) for s in SIDES)
            records[-1].update(hard_routing_correct=int(hard in valid),unguided_correct=int(unguided in valid),
                hard_routing_prediction=[corpus[s][hard[i]]['id'] for i,s in enumerate(SIDES)],
                unguided_prediction=[corpus[s][unguided[i]]['id'] for i,s in enumerate(SIDES)])
    ...
    if config['arm']=='h6':
        metrics['routing_ablation']={
            'soft_correct':sum(hits),
            'hard_correct':sum(r['hard_routing_correct'] for r in records),
            'unguided_correct':sum(r['unguided_correct'] for r in records),
            'soft_rescues_vs_hard':sum(r['hits']['R@1'] and not r['hard_routing_correct'] for r in records),
            'soft_regressions_vs_hard':sum(r['hard_routing_correct'] and not r['hits']['R@1'] for r in records),
            'policy':'same H6 checkpoint; soft normalized factorization primary; hard top1 service and raw-function scoring secondary'}
    result={'status':'complete' ,'arm':config['arm'],'task_level':'service' if service else 'function',
            ...
            'rows':n,'metrics':metrics,'paired_comparisons':{} if validation_only else paired_comparisons(run,groups,hits,config),
