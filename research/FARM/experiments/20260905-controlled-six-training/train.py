...
from experiment import RUNS, configuration, hierarchical_loss
from runtime import refresh_negatives, epoch_validation, joint_learning_curves, candidate_metrics
from checkpoint_loading import saved_config_overrides, assert_preserved_window, RELOAD_SENTINELS
...
def load_model(path=None, config=None):
    ...
    model.max_seq_length = config['max_seq_length'] if config else 512
    assert_preserved_window(model,path)
...
def side_batch(model, groups, indices, corpus, pools, side, epoch, config):
    ...
    if config['arm']=='h6':
        gold_services={corpus[p]['service'] for values in labels for p in values}
        candidates=[i for i,r in enumerate(corpus) if r['service'] in gold_services]
    else:
    ...
    return dict(q=q, d=d, qc=qc, dc=dc, scores=scores, loss=loss,
                candidates=candidates, local=local, labels=labels, positive=positive, allowed=allowed)
...
def function_step(models, heads, groups, indices, corpus, pools, phase, epoch, config):
    ...
    metrics = {'marginal_loss': float(loss.detach()), **candidate_metrics(cache[sides[0]],corpus[sides[0]])}
    if config['arm'] == 'f3':
        c = cache[phase]
    ...
    if config['arm']=='h6':
        c=cache[phase]
        loss=hierarchical_loss(c['scores'],heads[phase](c['q']),
             [corpus[phase][i]['service'] for i in c['candidates']],c['positive'],c['allowed'])
        metrics['hierarchical_loss']=float(loss.detach())
    if phase == 'joint':
...
def checkpoint(root, models, heads, optimizer, scheduler, step, fingerprint, epoch=None):
    name = ('epoch-%02d' % epoch) if epoch else ('checkpoint-%06d' % step)
    ...
    for side, model in models.items():
        ...
        model.eval()
        with torch.no_grad():
            sentinel = embed_features(model,prepare_features(model,RELOAD_SENTINELS,'query')).cpu()
...
def latest_checkpoint(root, fingerprint):
    options = sorted((p for p in root.iterdir() if (p.name.startswith('checkpoint-') or p.name.startswith('epoch-')) and (p/'complete.json').is_file()), key=lambda p:read(p/'complete.json')['step'])
    if not options:
        return None
    path = options[-1]
...
def train_phase(run, derived, phase, config, max_steps=0):
    ...
    code = {name:sha(Path(__file__).with_name(name)) for name in ('train.py','objectives.py','prepare.py','runtime.py','experiment.py','evaluate.py','checkpoint_loading.py')}
    ...
    models = {s:load_model(saved/s if saved else None,config) for s in model_sides}
    heads = nn.ModuleDict()
    if service:
        for s in SIDES:
            heads[s] = ServiceHead(prototypes[s],config['temperature'])
    elif config['arm'] in ('f3','h6'):
        heads[phase] = ServiceHead(prototypes[phase],config['temperature'])
    elif phase=='joint':
        heads['pair'] = PairInteraction(prototypes['trigger'].shape[1],config['interaction_rank'])
    heads.cuda().train()
    if not config['full_backbone_trainable']:
        raise ValueError('unsupported freezing policy; no silent ignored setting')
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer,round(total_steps*config['warmup_ratio']),total_steps)
    step = 0
    if saved:
        ...
        del state
    runtime={'trainable_backbone_parameters':sum(p.numel() for p in params),
             'trainable_head_parameters':sum(p.numel() for p in head_params),
             'parameter_dtypes':sorted({str(p.dtype) for p in params}),
             'effective_temperature':config['temperature'],
             'effective_sliding_window':next(iter(models.values()))[0].auto_model.config.sliding_window,
             'effective_max_length':next(iter(models.values())).max_seq_length,
             'warmup_steps':round(total_steps*config['warmup_ratio']),
             'optimizer_learning_rates':[g['initial_lr'] for g in optimizer.param_groups],
             'prompts':next(iter(models.values())).prompts}
    ...
    # Pool identity follows the epoch checkpoint even when resuming mid-epoch.
    if config['negative_refresh'] and step//steps_per_epoch:
        completed=min(step//steps_per_epoch,config['epochs']-1)
        pool_root=root/f'epoch-{completed:02d}'
        path=pool_root/'next_negatives.json'
        if step%steps_per_epoch and not path.exists():
            raise RuntimeError('mid-epoch resume is missing its exact negative pool')
        if path.exists():
            from runtime import source_identity
            saved_pool=read(path)
            if saved_pool['identity']['model']!=source_identity(pool_root,phase) or saved_pool['identity']['data_manifest_sha256']!=sha(derived/'manifest.json'):
                raise RuntimeError('resumed mining identity mismatch')
            pools[phase]=saved_pool['pools']
    def boundary(epoch_number):
        saved_epoch=checkpoint(root,models,heads,optimizer,scheduler,step,fingerprint,epoch_number)
        model=models['shared' if service else phase]
        epoch_validation(model,heads,phase,saved_epoch,derived,config)
        if config['negative_refresh'] and epoch_number in config['refresh_after_epochs']:
            pools[phase]=refresh_negatives(model,saved_epoch,groups,corpus[phase],phase,config,sha(derived/'manifest.json'))
    first_step = step
    ...
    while step < target_steps:
        ...
        gradient_norm = nn.utils.clip_grad_norm_(params+head_params,config['gradient_clip_norm'],error_if_nonfinite=True)
        optimizer.step()
        scheduler.step()
        step += 1
...
def main():
    ...
    run=args.run_root/('smoke' if args.smoke else 'arms')/args.run_id
    run.mkdir(parents=True,exist_ok=True)
    write(run/'config.json',config)
    phases=('shared',) if config['arm']=='s1' else ('joint',) if config['arm']=='f4' else SIDES
    if args.phase:
    ...
    try:
        ...
        if not args.max_steps and not args.phase:
            joint_learning_curves(run,args.derived,config)
            write(run/'progress.json',{'status':'evaluating','arm':config['arm'],'pid':os.getpid(),
        ...
        write(run/'failure.json',{'status':'failed','arm':config['arm'],'error':repr(exc),
