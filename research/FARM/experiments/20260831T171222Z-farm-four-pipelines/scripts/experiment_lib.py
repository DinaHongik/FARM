...
def config_from_path(path: Path) -> dict[str, Any]:
    ...
    if config["experiment_id"] not in {"e0", "e1", "e2", "e3"}:
...
def load_training_rows(
    ...
    if config["level"] == "service":
        ...
        rows = read_json(run_root / "derived_data" / "e3_hard_negatives" / f"{kind}.json")
    else:
...
def validate_training_rows(
    ...
    for row in rows:
        ...
        if config["hard_negatives"]:
            ...
            negative_urls = set(row.get("negative_urls") or [])
            if valid_urls & negative_urls:
                raise AssertionError(f"hard negative overlaps valid labels: {valid_urls & negative_urls}")
...
def validate_hardneg_derived(run_root: Path, data_root: Path) -> dict[str, Any]:
    root = run_root / "derived_data" / "e3_hard_negatives"
    manifest = read_json(root / "manifest.json")
    if manifest.get("status") != "passed" or manifest.get("dataset_id") != DATASET_ID:
        raise RuntimeError("invalid E3 derived manifest identity/status")
    if manifest.get("source_manifest_sha256") != sha256_file(data_root / "manifest.json"):
        raise RuntimeError("E3 source Dataset v2 manifest changed")
    for kind in ("trigger", "action"):
