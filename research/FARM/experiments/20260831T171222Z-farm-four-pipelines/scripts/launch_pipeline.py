...
def main() -> None:
    ...
    service_manifest = run_root / "derived_data" / "e0_service" / "manifest.json"
    hardneg_manifest = run_root / "derived_data" / "e3_hard_negatives" / "manifest.json"
    ...
    derived_hash = None
    if config["experiment_id"] == "e0":
        derived_hash = read_json(service_manifest)["derived_view_hash"]
    elif config["experiment_id"] == "e3":
        derived_hash = read_json(hardneg_manifest)["derived_view_hash"]
    else:
    ...
    pipeline_manifest = {
        ...
        "authorization_note": "current user explicitly reserved five GPUs and supplied matrix 0-4; stale repo notes listed 0-2; launcher rechecks occupancy and never kills processes",
    }
