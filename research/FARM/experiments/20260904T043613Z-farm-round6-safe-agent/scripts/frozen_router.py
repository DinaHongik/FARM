#!/usr/bin/env python3
"""Pure inference for the frozen Round3 recoverability router."""
...
def validate_router(value: Mapping[str, Any], *, candidate_sha256: str) -> dict[str, Any]:
    required = {"feature_names", "models", "threshold", "dataset_id", "calibration_split", "candidate_sha256"}
    ...
    for label in ("wrong", "covered"):
        ...
        if not isinstance(model, Mapping) or not all(len(model[key]) == len(FEATURE_NAMES) for key in ("mean", "scale", "coef")):
...
__all__ = ["FEATURE_NAMES", "feature_row", "routing_score", "validate_router"]
