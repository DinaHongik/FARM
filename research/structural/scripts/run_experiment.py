...
def execute(args):
    ...
    manifest = {'n':len(inputs), 'condition':'synthetic_smoke' if args.smoke else 'oracle_endpoints',
                ...
                'max_calls_per_case_arm':7, 'max_generated_tokens_per_call':2048,
    ...
    def status(state):
        ...
        save(args.output/'status.json', {'state':state, 'expected_cases':len(inputs),
             ...
             'model':args.model, 'semantic_binding_accuracy':None})
