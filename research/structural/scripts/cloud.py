...
class Cloud:
    ...
    def chat(self, identity, system, payload):
        body = {'model': self.model, 'stream': False, 'think': 'low', 'format': 'json',
                'messages': [{'role': 'system', 'content': system},
                             {'role': 'user', 'content': canonical(payload)}],
                'options': {'temperature': 0, 'seed': 9052026, 'num_predict': 2048}}
        ...
        result = {'request_sha256': signature, 'response_sha256': digest(raw),
                  ...
                  'done_reason': raw.get('done_reason'), 'request': body}
        save(path, result)
        return result
